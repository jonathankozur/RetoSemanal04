export default class PantallaInicial {
  ...
  inputJugador(nroJugador) {
    ...
    unLabel.setAttribute("for", "jugador" + nroJugador);
  ...
  agregarPartida(juegoSorteado, verPartida) {
    if (this.partidasGuardadas.length == 0) {
      this.modelo
        .querySelector(".inicio__partidas--titulo")
    ...
    partida_nombre.textContent =
      juegoSorteado.jugadores[0] + " vs " + juegoSorteado.jugadores[1];
